	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AppV1().Releases(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AppV1().Releases(namespace).Watch(options)
			},
		},
		&appv1.Release{},
		resyncPeriod,
...
func (f *releaseInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&appv1.Release{}, f.defaultInformer)
}

func (f *releaseInformer) Lister() listerV1.ReleaseLister {
	return listerV1.NewReleaseLister(f.Informer().GetIndexer())
}
