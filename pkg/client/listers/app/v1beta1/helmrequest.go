...
type HelmRequestLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.HelmRequest, err error)
...
// List lists all HelmRequests in the indexer.
func (s *helmRequestLister) List(selector labels.Selector) (ret []*v1beta1.HelmRequest, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.HelmRequest))
	})
	return ret, err
}
...
type HelmRequestNamespaceLister interface {
	...
	Get(name string) (*v1beta1.HelmRequest, error)
...
// List lists all HelmRequests in the indexer for a given namespace.
func (s helmRequestNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.HelmRequest, err error) {
...
// Get retrieves the HelmRequest from the indexer for a given namespace and name.
func (s helmRequestNamespaceLister) Get(name string) (*v1beta1.HelmRequest, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("helmrequest"), name)
	}
	return obj.(*v1beta1.HelmRequest), nil
}
