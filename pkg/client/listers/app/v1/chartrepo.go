...
type ChartRepoLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ChartRepo, err error)
...
// List lists all ChartRepos in the indexer.
func (s *chartRepoLister) List(selector labels.Selector) (ret []*v1.ChartRepo, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ChartRepo))
	})
	return ret, err
}
...
type ChartRepoNamespaceLister interface {
	...
	Get(name string) (*v1.ChartRepo, error)
...
// List lists all ChartRepos in the indexer for a given namespace.
func (s chartRepoNamespaceLister) List(selector labels.Selector) (ret []*v1.ChartRepo, err error) {
...
// Get retrieves the ChartRepo from the indexer for a given namespace and name.
func (s chartRepoNamespaceLister) Get(name string) (*v1.ChartRepo, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("chartrepo"), name)
	}
	return obj.(*v1.ChartRepo), nil
}
